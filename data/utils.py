...
def upsample(trial, new_hz, old_hz):
    ...
    trial.x = interp1d(points, trial.x.reshape(1, -1), kind='cubic')(new_points).reshape(-1)
    trial.y = interp1d(points, trial.y.reshape(1, -1), kind='cubic')(new_points).reshape(-1)
    return trial
...
def get_stats(series):
    try:  # DataFrame
        ...
        x_vals = series.apply(lambda x: x[:, 0])
        y_vals = series.apply(lambda x: x[:, 1])

    stat_df = pd.DataFrame(
        {'x_maxs': x_vals.apply(lambda x: np.nanmax(x)).describe()})
    stat_df['x_mins'] = x_vals.apply(lambda x: np.nanmin(x)).describe()

    stat_df['y_maxs'] = y_vals.apply(lambda x: np.nanmax(x)).describe()
    stat_df['y_mins'] = y_vals.apply(lambda x: np.nanmin(x)).describe()
    return stat_df
